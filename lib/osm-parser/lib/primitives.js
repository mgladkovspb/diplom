...
util.inherits(Primitives, Transform);
...
Primitives.prototype._transform = function(chunk, encoding, done) {
    var stream = this;
    zlib.inflate(chunk.data, (err, data) => {
        var block = proto.osmMesage.PrimitiveBlock.decode(data);
        var opts = {
        ...
        block.primitivegroup.forEach(function(group) {
            if (group.dense) {
                let nodes = parseDenseNodes(group.dense, opts);
                stream.emit('node', nodes);
            }
            let ways = [];
            group.ways.forEach(function(way) {
                ways.push(parseWay(way, opts));
            });
            if(ways.length > 0)
            stream.emit('way', ways);

            let relations = [];
            group.relations.forEach(function(relation) {
                relations.push(parseRelation(relation, opts));
            });
            if(relations.length > 0)
            stream.emit('relation', relations);
...
function decodeStringtable (objects) {
